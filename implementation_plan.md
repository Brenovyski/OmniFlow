...
**Runtime gotchas discovered during implementation (the spec was wrong on these):**

1. **No `GET /items` listing endpoint.** The spec/plan assumed `pluggy.fetchItems()` would return all items shared with our app via Data Passport. It doesn't exist on the SDK and the Pluggy REST API has no equivalent. Items can only be fetched by ID (`fetchItem(id)`). This means OmniFlow needs the user to provide each Pluggy item ID manually — implemented as the **Connect Pluggy item** dialog (Settings → Sources page header, alongside Sync from Pluggy and Add source). The dialog calls `pluggy-sync` with `{ connectItemId }` which validates the item via `fetchItem`, runs `mergeSource` (insert), then proceeds with the normal account/holding/transaction sync.
2. **`fetchTransactions` takes accountId, NOT itemId.** Same for `fetchAllTransactions`. The plan called both with itemId — wrong. Sync now iterates each Pluggy account inside an item and calls `fetchTransactions(accountId, { from, page, pageSize })` per account.
3. **The cursor variant `fetchAllTransactions` rejects the `from` filter** — only the page-based `fetchTransactions(accountId, { from, page, pageSize })` accepts it. We use the page variant with explicit pagination loops.
4. **Pluggy doesn't HMAC-sign webhook bodies.** Per their docs, auth is via custom headers configured at webhook-create time. The dashboard webhook UI doesn't allow setting custom headers, so we register the webhook via Pluggy's `POST /webhooks` API with `Authorization: Bearer <PLUGGY_WEBHOOK_SECRET>` (one-shot script: `node scripts/register-pluggy-webhook.mjs`). The webhook handler verifies the static header on every delivery. The spec's Implementation TBD #1 (HMAC algorithm) is therefore moot.
5. **"MeuPluggy" is Pluggy's umbrella connector** for Open Finance — `pluggyItem.connector.name` returns "MeuPluggy" regardless of which actual bank backs the item. The bank info is on the per-account `name` / `marketingName`. Auto-naming the source as `connector.name` produces unhelpful "MeuPluggy" duplicates; the user renames via the existing source-edit dialog (working) or we add a heuristic that derives a better name from the first account's marketingName during the initial sync (open issue, see below).
6. **`sources` does NOT have a `user_edited_fields` column** — by design, none of its fields are mergeable in the Pluggy sense (status / connector_id / last_synced_at are bank-truth; user-editable name / color come from the source-edit dialog and aren't re-set from Pluggy after first insert). `mergeSource` does not touch `user_edited_fields`. (Initial mistake of trying to set it on insert produced "Could not find the column in schema cache" — fixed.)
7. **Edge Function 500s without a body are opaque.** Both functions wrap their handler in a top-level `try { ... } catch` that surfaces uncaught error messages + stack traces in the response body, otherwise the Supabase gateway swallows them as `EDGE_FUNCTION_ERROR`. Keep this pattern when extending.

**Open issues remaining at end of session (Phase F unfinished):**

After Phases A–E plus Phase C deploy + the Connect Pluggy item dialog, an end-to-end sync against the user's two real connected items returned `transactionsUpserted=0`, `holdingsUpserted=0`, `accountsUpserted=5` (3 OmniFlow accounts after CC-link consolidation). The remaining bugs to chase next session:

1. **Transactions still 0 even with the `from`-on-page-variant fix.** The likely root cause: `pluggy_last_synced_at` got stamped on the source even on the earlier failed runs (we update it inside the per-target try block before the transaction loop completes). So second-and-later syncs query Pluggy with `from = last_synced - 1 day` ≈ "today only", missing the 90-day window. Fix: don't update `pluggy_last_synced_at` until the per-account transaction pass completes without errors. Alternative: reset `pluggy_last_synced_at = NULL` on the existing sources to force a fresh 90-day pull on the next sync. Verify by logging Pluggy's response page count for one debug run.
2. **Holdings are always skipped.** `mergeHolding` requires the source to have an account with `type='brokerage'`. But Pluggy returns BTG's investment account with `subtype='CHECKING_ACCOUNT'`, so it lands as `type='checking'` in OmniFlow — no brokerage row, every `mergeHolding` returns `skipped_no_account`. Fix options: (a) detect "investment" in `mergeBankAccount` via account name regex (`/invest/i`) or marketingName and re-classify `type='brokerage'`, or (b) auto-create a shadow brokerage account when a Pluggy investment lands without an existing brokerage under that source.
3. **Source auto-naming** — open optional improvement. Use the first account's marketingName / institution-derived name on initial insert instead of the generic "MeuPluggy" connector name. Manual rename via dialog already works as a fallback.
4. **BTG `credit_limit_cents=0` from Pluggy** — upstream issue, not ours. Pluggy's connector for BTG didn't include a CC limit in its response (the user's pre-Pluggy R$5,700 entry was manual). Verify by inspecting raw `creditData` from the SDK for that item; document for the user.

Once 1 + 2 land and a fresh sync produces non-zero transactions + non-zero holdings, the rest of the smoke checklist (sticky edit survives re-sync, soft-delete doesn't resurrect, webhook events fire, source picker disables Pluggy entries on the new-transaction form) can run end-to-end. Then the Phase F wrap-up (flip `[~]` Step 7.5 → `[x]`, final commit) closes out the step.
