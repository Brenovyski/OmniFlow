...
Step 6b (dashboard cashflow + accounts + net worth) is in. Done so far:
...
Next step (step 7) is **Categories page** — card grid with colored icon, name, type chip, and live transaction count; CRUD via dialogs; delete blocks when count > 0. See `implementation_plan.md` step 7.
